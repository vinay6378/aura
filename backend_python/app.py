...
app.config['JWT_SECRET_KEY'] = os.environ.get('JWT_SECRET_KEY', 'aura_super_secure_jwt_secret_key_2026_minimum_32_bytes')
...
if os.environ.get('FLASK_ENV') == 'production':
    ...
    # For local development, don't enforce HTTPS
...
def create_contact():
    ...
    try:
        ...
        # Broadcast new contact to admin panel via WebSocket
        broadcast_new_contact({
            'contact_id': contact_id,
            'name': sanitized_data['name'],
            'email': sanitized_data['email'],
            'service': sanitized_data['service'],
            'subject': sanitized_data['subject'],
            'status': 'pending',
            'created_at': str(timedelta(hours=24))
        })
...
def auth():
    ...
    try:
        ...
        return handle_error(e, 'Authentication failed')

def login():
    """Login with bcrypt password verification"""
    try:
        # Get raw data without action field for validation
        ...
        email = sanitize_input(validated_data['email'])
...
@socketio.on('connect')
def handle_connect():
    emit('connected', {'status': 'Connected to AURA Digital Admin Panel'})

@socketio.on('join_admin_room')
def handle_join_admin_room(data):
    room = data.get('room', 'admin')
    join_room(room)
    emit('joined_room', {'room': room, 'status': 'Joined admin room'})

@socketio.on('leave_admin_room')
def handle_leave_admin_room(data):
    room = data.get('room', 'admin')
    leave_room(room)
    emit('left_room', {'room': room, 'status': 'Left admin room'})

# Broadcast new contact submission to admin panel
def broadcast_new_contact(contact_data):
    socketio.emit('new_contact', contact_data, room='admin')

# Broadcast stats update to admin panel
def broadcast_stats_update(stats_data):
    socketio.emit('stats_update', stats_data, room='admin')

# Broadcast contact status update
def broadcast_contact_update(contact_data):
    socketio.emit('contact_updated', contact_data, room='admin')
...
def root():
    return jsonify({
        'message': 'AURA Digital Backend API',
        'version': '2.0.0',
        'status': 'running',
        'endpoints': {
            'auth': '/api/auth',
